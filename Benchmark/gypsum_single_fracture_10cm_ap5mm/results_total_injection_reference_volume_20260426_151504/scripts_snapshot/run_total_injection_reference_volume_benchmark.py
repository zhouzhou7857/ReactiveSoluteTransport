...
RUN_TAG = f"benchmark_gypsum_single_fracture_10cm_ap5mm_total_injection_reference_volume_{RUN_TIMESTAMP}"
RUN_ROOT = BENCH_DIR / f"results_total_injection_reference_volume_{RUN_TIMESTAMP}"
...
GYPSUM_RATE_MOL_PER_M2_PER_S = 4.0e-5
...
def compute_setup(travel_time: float) -> TravelTimeSetup:
    ...
    return TravelTimeSetup(
        ...
        gypsum_volume_m3=vref,
        gypsum_moles=vref / GYPSUM_MOLAR_VOLUME_M3_PER_MOL,
...
def phreeqc_step_list(max_time: float) -> list[float]:
    ...
    return [float(step) for step in base_steps if step <= max_time]
...
def generate_phreeqc_input(setup: TravelTimeSetup) -> tuple[Path, Path, Path]:
    ...
    initial_gypsum_cm3 = setup.gypsum_volume_m3 * 1.0e6
    text = f"""TITLE Total-injection reference gypsum kinetics for t_cross={setup.travel_time_s:.6e} s
#------------------------------------------------------------------------------
...
#   - One PHREEQC cumulative law F_ref(t) is shared by all PT particles
#------------------------------------------------------------------------------
...
# Gypsum solid = {setup.gypsum_volume_m3:.12e} m3 (1:1 solid:water volume assumption)
# Gypsum moles = {setup.gypsum_moles:.12e} mol
...
-start
  ...
  30 area = PARM(2)
  40 rate = PARM(1) * area * (1 - omega)
  50 moles = rate * TIME
  60 IF (moles > 0 AND moles > M) THEN moles = M
  70 GOTO 200
  100 moles = 0
  200 SAVE moles
-end
...
KINETICS 1
Gypsum
    -parms  {GYPSUM_RATE_MOL_PER_M2_PER_S:.12e}  {setup.reactive_area_m2:.12e}
    -m0     {setup.gypsum_moles:.12e}
    -m      {setup.gypsum_moles:.12e}
    -tol    1e-8
    -steps  {steps}
    -step_divide 1e-6
    -bad_step_max 50000
...
USER_PUNCH
    -headings Time_s pH SI_Gypsum V_Gyp_cm3 DV_Gyp_cm3
-start
  10 PUNCH TOTAL_TIME
  20 PUNCH -LA("H+")
  30 PUNCH SI("Gypsum")
  ...
  50 PUNCH v_gyp
  60 PUNCH {initial_gypsum_cm3:.12e} - v_gyp
-end
END
"""
...
def fit_reference_curve(travel_time: float, phreeqc_data: dict[str, np.ndarray], csv_path: Path) -> Fast2Fit:
    ...
    dv_m3 = np.maximum(phreeqc_data["DV_Gyp_cm3"] * 1.0e-6, 0.0)
    mask = (time_s > 0.0) & np.isfinite(time_s) & np.isfinite(dv_m3)
    t_fit = time_s[mask]
    y_fit = dv_m3[mask]
    if t_fit.size == 0:
        raise RuntimeError(f"No valid PHREEQC data to fit for t_cross={travel_time}")
    a2_guess = max(float(y_fit[-1]), 1.0e-18)
    k2_guess = min(1.0 / max(travel_time, 1.0), 1.0e-3)
    linear_guess = max(y_fit[-1] / max(t_fit[-1], 1.0) * 0.05, 0.0)
    popt, _ = curve_fit(
        dv_model,
        t_fit,
        y_fit,
    ...
    pred = dv_model(t_fit, *popt)
    rmse = float(np.sqrt(np.mean((pred - y_fit) ** 2)))
    denom = float(np.sum((y_fit - np.mean(y_fit)) ** 2))
    r2 = 1.0 - float(np.sum((pred - y_fit) ** 2)) / denom if denom > 0.0 else 1.0
    with csv_path.open("w", newline="", encoding="ascii") as f:
        writer = csv.writer(f)
        writer.writerow(["Time_s", "DV_phreeqc_m3", "DV_fit_m3"])
        for t_value, dv_value, fit_value in zip(t_fit, y_fit, pred):
            writer.writerow([f"{t_value:.12e}", f"{dv_value:.12e}", f"{fit_value:.12e}"])
    return Fast2Fit(
        a2_m3=float(popt[0]),
...
def compute_case_metrics(row: dict[str, str]) -> dict[str, float | str]:
    ...
    return {
        ...
        "reference_dissolved_volume_m3": float(row["reference_dissolved_volume_m3"]),
...
def plot_f_ref_curves(phreeqc_records: list[dict[str, object]]) -> None:
    ...
    ax.set_ylabel(r"Cumulative Dissolved Gypsum Volume [$10^{-6}$ m$^3$]")
    ax.set_title(r"PHREEQC Reference Curves $F_{ref}(t)$")
...
def plot_aperture_evolution(summary_rows: list[dict[str, object]]) -> None:
    ...
    for row in sorted(summary_rows, key=lambda item: float(item["travel_time_s"])):
        ...
        max_aperture = np.array([np.max(data[:, 4]) for _, data in snapshots], dtype=float) * 1.0e3
        label = rf"$t_{{cross}}={sci_label(float(row['travel_time_s']))}$ s"
        axes[0].plot(times, mean_aperture, lw=1.8, label=label)
        axes[1].plot(times, max_aperture, lw=1.8, label=label)
    for ax, title in zip(axes, ["Mean aperture", "Max aperture"]):
        ax.axhline(APERTURE * 1.0e3, color="#666666", lw=1.0, ls="--")
...
def plot_total_dissolved_volume(summary_rows: list[dict[str, object]]) -> None:
    ...
    for row in sorted(summary_rows, key=lambda item: float(item["travel_time_s"])):
        ...
        for time_value, data in snapshots:
            ...
            dissolved = float(np.sum(2.0 * delta_b * lengths * THICKNESS))
            times.append(time_value)
            dissolved_values.append(dissolved)
        ax.plot(times, np.array(dissolved_values) * 1.0e6, lw=1.8, label=rf"$t_{{cross}}={sci_label(float(row['travel_time_s']))}$ s")
    ax.set_xlabel("Simulation Time [s]")
    ax.set_ylabel(r"Accumulated Dissolved Gypsum Volume [$10^{-6}$ m$^3$]")
    ax.set_title("Accumulated Mineral Volume Change Over Time (n=100)")
    ax.grid(True, alpha=0.25)
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "fig_total_dissolved_volume_n100.png", dpi=220)
    plt.close(fig)
...
def plot_old_vs_new(summary_rows: list[dict[str, object]]) -> None:
    ...
    new_max = [float(new_by_tt[tt]["max_delta_aperture_mm"]) for tt in travel_times]
    old_vol = [float(old_by_tt[tt]["dissolved_volume_m3"]) * 1.0e6 for tt in travel_times]
    new_vol = [float(new_by_tt[tt]["dissolved_volume_m3"]) * 1.0e6 for tt in travel_times]

    fig, axes = plt.subplots(2, 1, figsize=(8.2, 8.0), sharex=True)
    axes[0].bar(x - width / 2.0, old_max, width=width, label="Old per-particle $F_{V_p}(t)$")
    axes[0].bar(x + width / 2.0, new_max, width=width, label="New total-volume $F_{ref}(t)$")
    axes[0].set_ylabel("Max Aperture Increase [mm]")
    axes[0].grid(True, axis="y", alpha=0.25)
    axes[0].legend(frameon=False)

    axes[1].bar(x - width / 2.0, old_vol, width=width, label="Old per-particle $F_{V_p}(t)$")
    axes[1].bar(x + width / 2.0, new_vol, width=width, label="New total-volume $F_{ref}(t)$")
    axes[1].set_ylabel(r"Dissolved Volume [$10^{-6}$ m$^3$]")
    ...
    for setup in setups:
        ...
        lines.extend(
            [
                ...
                f"- `Gypsum volume = {setup.gypsum_volume_m3:.12e} m^3`",
                f"- `Gypsum moles = {setup.gypsum_moles:.12e} mol`",
        ...
        for row in sorted(n100_rows, key=lambda item: float(item["travel_time_s"])):
            lines.extend(
                [
                    ...
                    f"- `dissolved_volume = {float(row['dissolved_volume_m3']):.12e} m^3`",
                    f"- `max_delta_aperture = {float(row['max_delta_aperture_mm']):.12e} mm`",
