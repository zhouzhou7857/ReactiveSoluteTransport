...
// The current coupling uses a fixed PHREEQC reference cumulative
// mineral-volume law F_ref(t), while legacy concentration-based values are
// retained only for deprecated inactive helpers. Particle increments are
// scaled as DeltaV_particle = DeltaV_ref * V_particle / Vref. If Vref is the
// total injected solution volume and particles have equal representative
// volumes, each particle carries weight 1/Np.
// Active default reference law:
// F_ref(t) =
//   A1 * (1 - exp(-k1 * t)) +
//   A2 * (1 - exp(-k2 * t)) +
//   L  * t
...
// Set the cumulative PHREEQC-derived reference law:
