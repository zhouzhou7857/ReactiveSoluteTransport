...
double RATIONAL_PRECIP_PINF = 0.0;
double RATIONAL_PRECIP_WEIGHT = 0.0;
double RATIONAL_PRECIP_A10_RATE = 0.0;
double RATIONAL_PRECIP_A2_RATE = 0.0;
...
bool USE_RATIONAL_PRECIPITATION_LAW = false;
bool USE_VP_WIDTH_CORRECTION = false;
bool USE_EFFECTIVE_DIFFUSION_HEIGHT_FACTOR = false;
bool CHEMISTRY_DEBUG_LOGGING = false;

// Current chemistry core:
// the active path uses particle age and a fixed PHREEQC-derived cumulative
// mineral-volume law F_ref(t). Particle representative volume is only a
// weight relative to the PHREEQC reference water volume Vref.
...
// Not used by the current PHREEQC-derived F_ref(t) coupling path.
...
void ResetChemistryParametersToDefaults()
{
	...
	RATIONAL_PRECIP_PINF = 0.0;
	RATIONAL_PRECIP_WEIGHT = 0.0;
	RATIONAL_PRECIP_A10_RATE = 0.0;
	RATIONAL_PRECIP_A2_RATE = 0.0;
	...
	USE_RATIONAL_PRECIPITATION_LAW = false;
...
{
	RATIONAL_PRECIP_PINF = pinf;
	RATIONAL_PRECIP_WEIGHT = std::max(0.0,std::min(1.0,weight));
	RATIONAL_PRECIP_A10_RATE = std::max(0.0,a10_rate);
	RATIONAL_PRECIP_A2_RATE = std::max(0.0,a2_rate);
	USE_RATIONAL_PRECIPITATION_LAW = true;
}

// Set the PHREEQC reference water volume Vref used in:
// scale = V_particle / Vref.
// In the current benchmark Vref is the total injected supersaturated water
// volume, not a per-particle water volume.
...
// PHREEQC-derived chemistry functions.
// The default law is a legacy two-exponential cumulative DeltaV(t). The
// precipitation benchmark activates the rational law below through
// RST_CHEM_MODE=rational_precipitation:
// F_ref(t) = Pinf * [1 - w*(1+a10*t)^(-1/9) - (1-w)*(1+a2*t)^(-1)].
// Pinf is signed. Negative F_ref denotes mineral precipitation and therefore
// decreases aperture through the unchanged volume-to-aperture map.
double GetCumulativeMineralVolumeChange(double t_particle)
{
	if (t_particle<=0.0){return 0.0;}
	double t = std::max(0.0,t_particle);
	if (USE_RATIONAL_PRECIPITATION_LAW){
		double high_order = std::pow(1.0 + RATIONAL_PRECIP_A10_RATE*t,-1.0/9.0);
		double low_order = 1.0/(1.0 + RATIONAL_PRECIP_A2_RATE*t);
		double bracket = 1.0 - RATIONAL_PRECIP_WEIGHT*high_order
			- (1.0 - RATIONAL_PRECIP_WEIGHT)*low_order;
		return RATIONAL_PRECIP_PINF*bracket;
	}
...
// Particle-specific volume scaling relative to the PHREEQC reference water:
// scale = V_particle / Vref by default. If Vref is the total injected solution
// volume and particles have equal representative volumes, each particle carries
// weight 1/Np. Optional correction flags modify this default only when enabled.
...
{
	// Segment chemistry contribution:
	// F_ref(t) is a fixed PHREEQC reference cumulative mineral-volume law.
	// DeltaV_particle = DeltaV_ref * V_particle / Vref by default. If Vref is
	// the total injected solution volume and particles have equal representative
	// volumes, each particle carries weight 1/Np.
	...
	double scale = ComputeParticleVolumeScalingFactor(particle_representative_volume,aperture_height);
	return delta_v_ref*scale;
}

double ComputeApertureChangeFromMineralVolume(double mineral_volume_change,double segment_length)
{
	// Geometry mapping for a 2D segment with the configured out-of-plane
	// thickness and symmetric two-wall dissolution/precipitation.
