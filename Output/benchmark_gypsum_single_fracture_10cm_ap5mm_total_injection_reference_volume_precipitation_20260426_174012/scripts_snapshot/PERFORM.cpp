...
{
	// Chemistry mode is selected at runtime so benchmark scripts can reuse the
	// same executable. The current gypsum precipitation benchmark uses
	// RST_CHEM_MODE=rational_precipitation with a total-injection-volume Vref.
	...
	else if (chemistry_mode=="rational_precipitation"){
		// Supersaturated gypsum precipitation fit:
		// F_ref(t)=Pinf*[1-w*(1+a10*t)^(-1/9)-(1-w)*(1+a2*t)^(-1)].
		// Pinf is normally negative so the aperture update represents
		// precipitation-driven aperture reduction.
		double pinf = 0.0;
		double weight = 0.0;
		double a10_rate = 0.0;
		double a2_rate = 0.0;
		ReadEnvironmentDouble("RST_RATIONAL_PINF",pinf);
		ReadEnvironmentDouble("RST_RATIONAL_WEIGHT",weight);
		ReadEnvironmentDouble("RST_RATIONAL_A10",a10_rate);
		ReadEnvironmentDouble("RST_RATIONAL_A2",a2_rate);
		ConfigureRationalPrecipitationLaw(pinf,weight,a10_rate,a2_rate);
	}
...
int main(int argc, char **argv) {
	...
	if (USE_RATIONAL_PRECIPITATION_LAW){
		cout << "Chemistry DeltaV(t) = Pinf*(1-w*(1+a10*t)^(-1/9)-(1-w)*(1+a2*t)^(-1))" << endl;
	}
	else{
	...
	cout << "Chemistry parameters: A1=" << DELTA_V_FAST1_AMPLITUDE
	     ...
	     << " rational_pinf=" << RATIONAL_PRECIP_PINF
	     << " rational_w=" << RATIONAL_PRECIP_WEIGHT
	     << " rational_a10=" << RATIONAL_PRECIP_A10_RATE
	     << " rational_a2=" << RATIONAL_PRECIP_A2_RATE
