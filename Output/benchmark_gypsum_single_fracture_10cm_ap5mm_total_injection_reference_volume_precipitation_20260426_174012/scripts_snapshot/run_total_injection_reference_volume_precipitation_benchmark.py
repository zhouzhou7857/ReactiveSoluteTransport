...
RUN_TIMESTAMP = os.environ.get("RST_RUN_TIMESTAMP", datetime.now().strftime("%Y%m%d_%H%M%S"))
...
GYPSUM_PRECIP_K1_MOL_PER_M2_PER_S = 1.0
GYPSUM_PRECIP_K2_MOL_PER_M2_PER_S = 5.0e-7
...
class Fast2Fit:
    ...
    rational_pinf_m3: float
    rational_weight: float
    rational_a10_s_inv: float
    rational_a2_s_inv: float
    rmse_m3: float
    r2: float
...
# empirical precipitation law:
#   rate = -A * (k1 * (sqrt(SR)-1)^10 + k2 * (sqrt(SR)-1)^2)
# k1           = {GYPSUM_PRECIP_K1_MOL_PER_M2_PER_S:.12e} mol/m2/s
# k2           = {GYPSUM_PRECIP_K2_MOL_PER_M2_PER_S:.12e} mol/m2/s
...
-start
  10 omega = SR("Gypsum")
  20 IF omega <= 1 THEN SAVE 0 : END
  30 A = PARM(1)
  40 k1 = PARM(2)
  50 k2 = PARM(3)
  60 x = SQRT(omega) - 1
  70 rate = -A * (k1 * x^10 + k2 * x^2)
  80 SAVE rate * TIME
-end
...
Gypsum
    -m      0.0
    -m0     0.0
    -parms  {setup.reactive_area_m2:.12e}  {GYPSUM_PRECIP_K1_MOL_PER_M2_PER_S:.12e}  {GYPSUM_PRECIP_K2_MOL_PER_M2_PER_S:.12e}
    -tol    1e-8
    -steps  {steps}
    -cvode  true
...
def dv_model(time_s: np.ndarray, pinf: float, weight: float, a10: float, a2: float) -> np.ndarray:
    return pinf * (
        1.0
        - weight * (1.0 + a10 * time_s) ** (-1.0 / 9.0)
        - (1.0 - weight) * (1.0 + a2 * time_s) ** (-1.0)
    )
...
def fit_reference_curve(travel_time: float, phreeqc_data: dict[str, np.ndarray], csv_path: Path) -> Fast2Fit:
    ...
    total_guess = max(float(y_fit[-1]) * 1.25, 1.0e-18)
    fast_time = max(float(t_fit[min(3, t_fit.size - 1)]), 1.0)
    slow_time = max(float(t_fit[-1]) * 0.1, 1.0)
    sigma = np.maximum(y_fit, max(float(y_fit[-1]) * 1.0e-4, 1.0e-18))
    popt, _ = curve_fit(
        dv_model,
        t_fit,
        y_fit,
        p0=[total_guess, 0.5, 1.0 / fast_time, 1.0 / slow_time],
        bounds=([float(y_fit[-1]), 0.0, 0.0, 0.0], [np.inf, 1.0, np.inf, np.inf]),
        sigma=sigma,
    ...
    return Fast2Fit(
        ...
        k2_s_inv=0.0,
        linear_m3_per_s=0.0,
        rational_pinf_m3=-float(popt[0]),
        rational_weight=float(popt[1]),
        rational_a10_s_inv=float(popt[2]),
        rational_a2_s_inv=float(popt[3]),
        rmse_m3=rmse,
        r2=r2,
    )
...
def run_pt_case(
    ...
    run_env = {
        **os.environ,
        "RST_CHEM_MODE": "rational_precipitation",
        "RST_DELTA_V_A1": f"{fit.a1_m3:.12e}",
        "RST_DELTA_V_K1": f"{fit.k1_s_inv:.12e}",
        "RST_DELTA_V_A2": f"{fit.a2_m3:.12e}",
        "RST_DELTA_V_K2": f"{fit.k2_s_inv:.12e}",
        "RST_DELTA_V_L": f"{fit.linear_m3_per_s:.12e}",
        "RST_RATIONAL_PINF": f"{fit.rational_pinf_m3:.12e}",
        "RST_RATIONAL_WEIGHT": f"{fit.rational_weight:.12e}",
        "RST_RATIONAL_A10": f"{fit.rational_a10_s_inv:.12e}",
        "RST_RATIONAL_A2": f"{fit.rational_a2_s_inv:.12e}",
...
def integrated_reference_mineral_volume_change(setup: TravelTimeSetup, fit: Fast2Fit) -> float:
    """Integrate the signed reference mineral-volume law over the injection window."""
    t_end = GLOBAL_INJECTION_TIME
    time_grid = np.linspace(0.0, t_end, 2001)
    cumulative = fit.rational_pinf_m3 * (
        1.0
        - fit.rational_weight * (1.0 + fit.rational_a10_s_inv * time_grid) ** (-1.0 / 9.0)
        - (1.0 - fit.rational_weight) * (1.0 + fit.rational_a2_s_inv * time_grid) ** (-1.0)
    )
    integral = float(np.trapezoid(cumulative, time_grid))
    return (setup.q_in_m3_per_s / setup.vref_m3) * integral
...
def compute_case_metrics(row: dict[str, str]) -> dict[str, float | str]:
    ...
    return {
        ...
        "fit_a1_m3": float(row["fit_a1_m3"]),
        "fit_k1_s_inv": float(row["fit_k1_s_inv"]),
        "fit_a2_m3": float(row["fit_a2_m3"]),
        "fit_k2_s_inv": float(row["fit_k2_s_inv"]),
        "fit_linear_m3_per_s": float(row["fit_linear_m3_per_s"]),
        "fit_rational_pinf_m3": float(row["fit_rational_pinf_m3"]),
        "fit_rational_weight": float(row["fit_rational_weight"]),
        "fit_rational_a10_s_inv": float(row["fit_rational_a10_s_inv"]),
        "fit_rational_a2_s_inv": float(row["fit_rational_a2_s_inv"]),
        "fit_rmse_m3": float(row["fit_rmse_m3"]),
        "fit_r2": float(row["fit_r2"]),
        "reference_mineral_volume_change_m3": float(
            row.get("reference_mineral_volume_change_m3", row.get("reference_dissolved_volume_m3", 0.0))
        ),
...
def plot_total_precipitated_volume(summary_rows: list[dict[str, object]]) -> None:
    fig, ax = plt.subplots(figsize=(8.0, 5.0))
    for row in sorted(summary_rows, key=lambda item: float(item["travel_time_s"])):
        if int(row["segments"]) != 100:
            continue
...
def plot_crossing_time_mechanism(summary_rows: list[dict[str, object]]) -> None:
    n100_rows = sorted(
        [row for row in summary_rows if int(row["segments"]) == 100],
        key=lambda item: float(item["travel_time_s"]),
    )
    if not n100_rows:
        return

    travel_times = np.array([float(row["travel_time_s"]) for row in n100_rows], dtype=float)
    v_inj = np.array([float(row["vref_m3"]) for row in n100_rows], dtype=float)
    max_aperture_reduction_mm = np.array(
        [-float(row["min_delta_aperture_mm"]) for row in n100_rows],
        dtype=float,
    )
    eta_segment = []
    for row in n100_rows:
        t_cross = float(row["travel_time_s"])
        segments = int(row["segments"])
        tau_segment = t_cross / float(segments)
        pinf_abs = abs(float(row["fit_rational_pinf_m3"]))
        weight = float(row["fit_rational_weight"])
        a10 = float(row["fit_rational_a10_s_inv"])
        a2 = float(row["fit_rational_a2_s_inv"])
        if pinf_abs <= 0.0:
            eta_segment.append(0.0)
            continue
        precip_abs = pinf_abs * (
            1.0
            - weight * (1.0 + a10 * tau_segment) ** (-1.0 / 9.0)
            - (1.0 - weight) * (1.0 + a2 * tau_segment) ** (-1.0)
        )
        eta_segment.append(max(0.0, min(1.0, precip_abs / pinf_abs)))
    eta_segment_arr = np.array(eta_segment, dtype=float)
    precip_proxy = v_inj * eta_segment_arr
    ...
    mechanism_csv = FIG_DIR / "crossing_time_mechanism_n100.csv"
    with mechanism_csv.open("w", newline="", encoding="ascii") as f:
        writer = csv.writer(f)
        writer.writerow(
            [
                "t_cross_s",
                ...
                "eta_inlet_segment",
                "v_inj_times_eta_m3",
                "max_aperture_reduction_mm",
            ]
        )
        for values in zip(travel_times, v_inj, eta_segment_arr, precip_proxy, max_aperture_reduction_mm):
    ...
    ax.plot(
        ...
        "s-",
        lw=2.0,
        label=r"$\eta_{seg}=P(t_{cross}/100)/P_\infty$",
    )
    ax.plot(
        travel_times,
        normalize(precip_proxy),
        "^-",
        lw=2.0,
        label=r"$V_{inj}\eta_{seg}$, normalized",
    )
    ax.set_xscale("log")
    ax.set_xlabel(r"Crossing Time $t_{cross}$ [s]")
    ax.set_ylabel("Normalized Mechanism Terms [-]")
    ...
    ax2.plot(
        ...
        color="#8c2d04",
        lw=2.0,
        label="Max aperture reduction [mm]",
    )
    ax2.set_ylabel("Max Aperture Reduction [mm]")
    ...
    caption = (
        r"$V_{inj}=Q_{in}t_{inj}$ decreases as $t_{cross}$ increases; "
        r"$\eta_{seg}=P(t_{cross}/100)/P_\infty$ is the inlet-segment precipitation completion; "
        "\n"
        r"$V_{inj}\eta_{seg}$ is a local precipitation proxy, and the measured max aperture reduction follows its intermediate peak."
    )
    fig.text(0.5, 0.035, caption, ha="center", va="bottom", fontsize=11)
    fig.tight_layout(rect=(0.0, 0.13, 1.0, 0.91))
    fig.savefig(FIG_DIR / "fig_crossing_time_mechanism_n100.png", dpi=220)
    plt.close(fig)
    ...
    for setup in setups:
        ...
        report_lines.extend(
            [
                ...
                f"- `rational Pinf = {float(phreeqc_record['rational_pinf_m3']):.12e} m^3`",
                f"- `rational w = {float(phreeqc_record['rational_weight']):.12e}`",
                f"- `rational a10 = {float(phreeqc_record['rational_a10_s_inv']):.12e} s^-1`",
                f"- `rational a2 = {float(phreeqc_record['rational_a2_s_inv']):.12e} s^-1`",
...
def write_readme(
    ...
    lines = [
        ...
        f"- Precipitation kinetics use `A={A_REF_M2:g} m2`, `k1={GYPSUM_PRECIP_K1_MOL_PER_M2_PER_S:g} mol/m2/s`, `k2={GYPSUM_PRECIP_K2_MOL_PER_M2_PER_S:g} mol/m2/s`.",
    ...
    for setup in setups:
        ...
        lines.extend(
            [
                ...
                f"- `Fit: Pinf = {float(fit['rational_pinf_m3']):.12e} m^3, w = {float(fit['rational_weight']):.12e}, a10 = {float(fit['rational_a10_s_inv']):.12e} s^-1, a2 = {float(fit['rational_a2_s_inv']):.12e} s^-1`",
            ]
        )
...
def main() -> None:
    ...
    for setup in setups:
        ...
        fit = fit_by_tt[setup.travel_time_s]
        reference_mineral_volume_change = integrated_reference_mineral_volume_change(setup, fit)
            ...
            case_index_rows.append(
                {
                    ...
                    "fit_a1_m3": fit.a1_m3,
                    "fit_k1_s_inv": fit.k1_s_inv,
                    "fit_a2_m3": fit.a2_m3,
                    "fit_k2_s_inv": fit.k2_s_inv,
                    "fit_linear_m3_per_s": fit.linear_m3_per_s,
                    "fit_rational_pinf_m3": fit.rational_pinf_m3,
                    "fit_rational_weight": fit.rational_weight,
                    "fit_rational_a10_s_inv": fit.rational_a10_s_inv,
                    "fit_rational_a2_s_inv": fit.rational_a2_s_inv,
                    "fit_rmse_m3": fit.rmse_m3,
                    "fit_r2": fit.r2,
                    "reference_mineral_volume_change_m3": reference_mineral_volume_change,
    ...
    plot_total_precipitated_volume(summary_rows)
    plot_crossing_time_mechanism(summary_rows)
